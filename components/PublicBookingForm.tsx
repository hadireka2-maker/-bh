...
const PublicBookingForm: React.FC<PublicBookingFormProps> = ({ 
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Persist to Supabase (clients, projects, transactions, leads, promo usage)
        try {
            ...
            await supabase.from('clients').insert({
                ...
                instagram: newClient.instagram ?? null,
                client_type: 'DIRECT',
                since: newClient.since,
                status: 'ACTIVE',
                last_contact: newClient.lastContact,
            ...
            await supabase.from('projects').insert({
                ...
                package_id: newProject.packageId,
                package_name: newProject.packageName,
            ...
            const newLeadId = `LEAD-FORM-${Date.now()}`;
            await supabase.from('leads').insert({
                ...
                location: newProject.location,
                whatsapp: null,
                notes: `Dikonversi otomatis dari formulir pemesanan publik. Proyek: ${newProject.projectName}. Klien ID: ${newClient.id}`,
                status: 'Dikonversi',
            ...
            if (promoCodeAppliedId) {
                // Try RPC if exists, fallback to update
                try { await supabase.rpc('increment_promo_usage', { pid: promoCodeAppliedId }); }
                catch {
                    const { data } = await supabase.from('promo_codes').select('usage_count').eq('id', promoCodeAppliedId).single();
                    const curr = (data as any)?.usage_count ?? 0;
                    await supabase.from('promo_codes').update({ usage_count: curr + 1 }).eq('id', promoCodeAppliedId);
                }
            }
        } catch (err) {

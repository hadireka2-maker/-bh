...
const PublicRevisionForm: React.FC<PublicRevisionFormProps> = ({ projects, teamMembers, onUpdateRevision }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const { upsertRevision } = await import('../services/revisions');
            await upsertRevision({
                ...
                freelancer_notes: formData.freelancerNotes,
                drive_link: formData.driveLink,
                completed_date: formData.status === RevisionStatus.COMPLETED ? new Date().toISOString() : null,
            });
        } catch (err) {
            console.error('Supabase update revision failed:', err);
        }
